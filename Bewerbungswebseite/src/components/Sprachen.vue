    ...
    <div class="projekt-container">
      ...
      <p class="info-text">
        ...
        <svg :viewBox="viewBox" class="cambridge-svg">
          <rect x="0" y="0" width="100%" height="100%" rx="18" fill="#fff" stroke="#e0e0e0" stroke-width="2" />
          ...
          <g v-if="!isMobile">
            ...
            <g v-for="bar in chartData" :key="bar.label">
              <rect :x="bar.x" :y="bar.y" :width="bar.width" :height="bar.height" fill="#e76f51" rx="6" />
              <text :x="bar.valueX" :y="bar.valueY" class="bar-value" text-anchor="middle">{{ Math.round(bar.animatedValue) }}</text>
          ...
          <g v-if="isMobile">
            <!-- Scale -->
            <text x="50%" y="40" class="scale-label" text-anchor="middle">Cambridge English Scale</text>
            <line x1="130" y1="55" x2="360" y2="55" class="scale-line" />
            <g v-for="score in [120, 140, 160, 180, 190]" :key="score">
                <line :x1="130 + (score - 120) / 70 * 230" y1="55" :x2="130 + (score - 120) / 70 * 230" y2="600" class="scale-line-dashed" />
                <text :x="130 + (score - 120) / 70 * 230" y="70" class="scale-number" text-anchor="middle">{{ score }}</text>
            </g>
            <!-- Bars and Labels -->
            <g v-for="bar in chartData" :key="bar.label">
              <rect :x="bar.x" :y="bar.y" :width="bar.width" :height="bar.height" fill="#e76f51" rx="6" />
              <text :x="bar.labelX" :y="bar.labelY" class="bar-label" text-anchor="end">{{ bar.label }}</text>
              <text :x="bar.valueX" :y="bar.valueY" class="bar-value" text-anchor="start">{{ Math.round(bar.animatedValue) }}</text>
            </g>
          </g>
        </svg>
      </div>
